...
class Qwen2_5_VLAttention(nn.Module):
    ...
    def forward(
        ...
        #!
            ...
            query_score, key_score, attn_output_temp, o_score = attn_weights_temp.squeeze().tolist(), attn_weights_temp.squeeze().tolist(), attn_output_temp, torch.sum(torch.abs(attn_output_temp), dim=1).squeeze().tolist() #!
        ...
        if early_layers:
            return attn_output, attn_weights, past_key_value, query_score, key_score, torch.sum(torch.abs(attn_output_temp), dim=1).squeeze().tolist(), o_score
        else:
...
class Qwen2_5_VLDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        # Self Attention
        hidden_states, self_attn_weights, present_key_value, q_score, k_score, v_score, o_score = self.self_attn(
            hidden_states=hidden_states,
            attention_mask=attention_mask,
            position_ids=position_ids,
            past_key_value=past_key_value,
...
class Qwen2_5_VLModel(Qwen2_5_VLPreTrainedModel):
    ...
    def forward(
        ...
        outputs, hidden_scores_fwd_up, hidden_scores_fwd_down, hidden_scores_q, hidden_scores_k, hidden_scores_v, hidden_scores_o = self.language_model(
            input_ids=None,
...
class Qwen2_5_VLForConditionalGeneration(GenerationMixinCustom, Qwen2_5_VLPreTrainedModel):
    ...
    def forward(
        ...
        def _aggregate_list_of_vectors(vs, backend="auto", reduce="sum"):
            """
            Aggregate a list of vectors into a single vector.

            Parameters
            ----------
            vs : list[Tensor | ndarray | list]
                Each element is a vector (1D or multi-dimensional) with the same shape.
            backend : {"auto", "torch", "numpy"}
                Backend used for aggregation.
            reduce : {"sum", "mean"}
                Reduction type across the list.
            ...
            """
            if len(vs) == 0:
                return vs  # early exit on empty input

            # Normalize to torch or numpy.
            if backend == "auto":
            ...
            if backend == "torch":
                if not isinstance(vs[0], torch.Tensor):
                    vs = [torch.as_tensor(x) for x in vs]
                stacked = torch.stack(vs, dim=0)
                return stacked.sum(dim=0) if reduce == "sum" else stacked.mean(dim=0)
            else:
                arr = np.asarray(vs)
                return arr.sum(axis=0) if reduce == "sum" else arr.mean(axis=0)

        if early_exit_layers is not None:
            # 1) For each layer, aggregate per-sample score vectors into a single vector.
            summed_data_fwd = {k: _aggregate_list_of_vectors(v, reduce="sum")
                               for k, v in hidden_scores_fwd_up.items()}
            summed_data_down = {k: _aggregate_list_of_vectors(v, reduce="sum")
                                for k, v in hidden_scores_fwd_down.items()}
            summed_data_q = {k: _aggregate_list_of_vectors(v, reduce="sum")
                             for k, v in hidden_scores_q.items()}
            summed_data_k = {k: _aggregate_list_of_vectors(v, reduce="sum")
                             for k, v in hidden_scores_k.items()}
            summed_data_v = {k: _aggregate_list_of_vectors(v, reduce="sum")
                             for k, v in hidden_scores_v.items()}
            summed_data_o = {k: _aggregate_list_of_vectors(v, reduce="sum")
                             for k, v in hidden_scores_o.items()}

            # 2) Combine scores (still as vectors).
            #    Each of summed_data_fwd/down/q/k/v/o[k] is a vector; shapes can differ across modules.

            # First collapse each module vector to a scalar per layer, used for ranking layers.
            def _vec_to_scalar(x, how="sum"):
                import numpy as np, torch
                if isinstance(x, torch.Tensor):
                    return x.sum().item() if how == "sum" else x.mean().item()
                else:
                    x = np.asarray(x)
                    return float(x.sum()) if how == "sum" else float(x.mean())

            layer_score_fwd = {k: _vec_to_scalar(v, "sum") for k, v in summed_data_fwd.items()}
            ...
            for i, early_exit_layer in enumerate(early_exit_layers):
                ...
                top_number_attn = max(1, int(top_ratio_atten * attn_len))

                def _as_np(x):
                    import numpy as np, torch
                    return x.detach().cpu().numpy() if isinstance(x, torch.Tensor) else np.asarray(x)

                fwd_up_vec = _as_np(summed_data_fwd[early_exit_layer])
                fwd_down_vec = _as_np(summed_data_down[early_exit_layer])
                q_vec = _as_np(summed_data_q[early_exit_layer])
                k_vec = _as_np(summed_data_k[early_exit_layer])
                v_vec = _as_np(summed_data_v[early_exit_layer])
                o_vec = _as_np(summed_data_o[early_exit_layer])

                activate_keys_fwd_up[early_exit_layer] = np.argsort(fwd_up_vec)[-top_number_ffn:][::-1]
                activate_keys_fwd_down[early_exit_layer] = np.argsort(fwd_down_vec)[-top_number_ffn:][::-1]
                activate_keys_q[early_exit_layer] = np.argsort(q_vec)[-top_number_attn:][::-1]
                activate_keys_k[early_exit_layer] = np.argsort(k_vec)[-top_number_attn:][::-1]
                activate_keys_v[early_exit_layer] = np.argsort(v_vec)[-top_number_attn:][::-1]
                activate_keys_o[early_exit_layer] = np.argsort(o_vec)[-top_number_attn:][::-1]
        ...
        if early_exit_layers is not None:
            return logits_dict, Qwen2_5_VLCausalLMOutputWithPast(
                        loss=loss,
                        logits=logits,
                        past_key_values=outputs.past_key_values,
                        hidden_states=outputs.hidden_states,
