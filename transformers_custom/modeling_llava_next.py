...
"""PyTorch Llava-NeXT model with optional activation capture."""
...
class LlavaNextCausalLMOutputWithPast(ModelOutput):
    ...
    fwd_up/fwd_down/q/k/v/o (all optional):
        Dict[int, torch.FloatTensor]: Per-layer activations captured via forward hooks when `return_hidden_scores=True`.
        Keys are layer indices. Tensors are moved to CPU and cast to float32 by default.
    """
    ...
    fwd_up: Optional[Dict[int, torch.FloatTensor]] = None
    fwd_down: Optional[Dict[int, torch.FloatTensor]] = None
    q: Optional[Dict[int, torch.FloatTensor]] = None
    k: Optional[Dict[int, torch.FloatTensor]] = None
    v: Optional[Dict[int, torch.FloatTensor]] = None
    o: Optional[Dict[int, torch.FloatTensor]] = None
...
class LlavaNextModel(LlavaNextPreTrainedModel):
    ...
    def __init__(self, config: LlavaNextConfig):
        ...
        self.text_model = CustomLlamaModel(config.text_config)
        self.language_model = self.text_model
    ...
    def forward(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        if isinstance(outputs, tuple) and len(outputs) == 7 and isinstance(outputs[1], dict):
            # print(outputs)
            outputs, hidden_scores_fwd_up, hidden_scores_fwd_down, hidden_scores_q, hidden_scores_k, hidden_scores_v, hidden_scores_o = outputs
            extra_scores = (hidden_scores_fwd_up, hidden_scores_fwd_down, hidden_scores_q, hidden_scores_k, hidden_scores_v, hidden_scores_o)
        else:
            outputs = outputs

        final =  LlavaNextModelOutputWithPast(
...
class LlavaNextForConditionalGeneration(LlavaNextPreTrainedModel, GenerationMixinCustom):
    ...
    # ===================== Helper: Activation hooks =====================
    def _locate_lm_layers(self, lm: nn.Module):
        """
        Try to locate the list of decoder layers in various llama-style models.
        """
        if hasattr(lm, "layers"):
            return lm.layers
        if hasattr(lm, "model") and hasattr(lm.model, "layers"):
            return lm.model.layers
        raise AttributeError("Cannot find decoder layers on the language model.")
    ...
    def _register_activation_hooks(
        ...
        """
        Register forward hooks on q/k/v/o and mlp up/down of LLaMA layers.
        Returns (hooks, buckets_dict) so caller can remove hooks later.
        Captured tensors are detached to CPU (float32) to reduce VRAM usage.
        """
        hooks = []

        # Buckets: Dict[str, Dict[int, Tensor]]
        buckets = {
            "q": {},
            "k": {},
            "v": {},
        ...
        def make_hook(store: Dict[int, torch.Tensor], layer_idx: int):
            def _hook(_module, _inp, out):
                try:
                    # detach to cpu and upcast for safety (half on CPU can be flaky on some builds)
                    store[layer_idx] = out.detach().float().cpu()
                except Exception:
                    # fallback: best-effort
                    store[layer_idx] = out
            return _hook

        layers = self._locate_lm_layers(lm)
        wanted = set(only_layers) if only_layers is not None else None
            ...
            if attn is not None:
                for proj_name, key in [("q_proj", "q"), ("k_proj", "k"), ("v_proj", "v"), ("o_proj", "o")]:
                    if hasattr(attn, proj_name):
                        ...
                        hooks.append(mod.register_forward_hook(make_hook(buckets[key], idx)))

            # MLP projections
            mlp = getattr(layer, "mlp", None)
            if mlp is not None:
                if hasattr(mlp, "up_proj"):
                    hooks.append(mlp.up_proj.register_forward_hook(make_hook(buckets["fwd_up"], idx)))
                if hasattr(mlp, "down_proj"):
                    hooks.append(mlp.down_proj.register_forward_hook(make_hook(buckets["fwd_down"], idx)))

        return hooks, buckets

    def _remove_hooks(self, hooks: List[torch.utils.hooks.RemovableHandle]):
        for h in hooks:
            try:
                h.remove()
            except Exception:
    ...
    def forward(
        ...
        **kwargs,
        ...
        return_hidden_scores (`bool`, *optional*):
            If True, capture per-layer activations of q/k/v/o and mlp up/down via forward hooks and return them.
        early_exit_layers (`List[int]`, *optional*):
            If your injected `language_model` supports early-exit/activation export, those indices will be
            forwarded. Otherwise they are used to *filter which layers to capture*, not to truly early-exit.
        """
        output_attentions = output_attentions if output_attentions is not None else self.config.output_attentions
        output_hidden_states = (
            output_hidden_states if output_hidden_states is not None else self.config.output_hidden_states
        ...
        # ========== Optional: set up hooks for activation capture ==========
        hooks = []
        buckets = None

        # Try to call extended language model API first (if user injected a custom model),
        # otherwise fallback to hook-based capture.
        extended_api_worked = False
        extended_outputs = None

        if return_hidden_scores or (early_exit_layers is not None):
            # Try extended forward first (duck-typing by trying kwargs)
            try:
                # We register hooks regardless; if extended call fails, we keep hooks.
                hooks, buckets = self._register_activation_hooks(self.model.language_model, early_exit_layers)
                # Some custom backbones accept these extra kwargs; we attempt to pass them through self.model(...) call.
                # If the inner language model doesn't accept them, our hooks still capture from vanilla LLaMA.
            except Exception:
                # If hook registration fails, we still try extended API route
                hooks = []
                buckets = None
        ...
        outputs = self.model(
            ...
            return_dict=True,
            cache_position=cache_position,
            # Pass-through optional keys for custom text models (ignored by HF LLaMA)
            early_exit_layers=early_exit_layers,
            return_hidden_scores=return_hidden_scores,
            activate_keys_fwd_up_set=activate_keys_fwd_up_set,
            activate_keys_fwd_down_set=activate_keys_fwd_down_set,
            activate_keys_q_set=activate_keys_q_set,
            activate_keys_k_set=activate_keys_k_set,
        ...
        #     extended_api_worked = True  # if custom text model consumed args and returned extra, we'll detect later
        # except TypeError:
        #     # HF LLaMA doesn't accept the extra kwargs; retry without them (hooks will do the job).
        #     outputs = self.model(
        #         input_ids,
        #         pixel_values=pixel_values,
        #         image_sizes=image_sizes,
        ...
        #     )
        # finally:
        #     # Always remove hooks right after forward
        #     if hooks:
        ...
        # If the injected custom model returned its own activation dicts in a tuple-like,
        # allow users to plug-and-play. (Best-effort compatibility with your reference style)

        base_out =  LlavaNextCausalLMOutputWithPast(
            loss=loss,
            logits=logits,
            past_key_values=outputs.past_key_values,
            hidden_states=outputs.hidden_states,
