...
class Qwen2MLP(nn.Module):
    ...
    def forward(self, x, early_layers=None, deactivate_down=None, deactivate_up=None):
        ...
        # !
        if early_layers:
            return down_proj, torch.sum(torch.abs(self.up_proj(x)), dim=1).squeeze().tolist(), torch.sum(
                torch.abs(self.up_proj(x)), dim=1).squeeze().tolist()
...
class Qwen2_5_VLAttention(nn.Module):
    ...
    def forward(
        ...
        if early_layers:
            attn_weights_temp = torch.matmul(query_states.transpose(2, 3).unsqueeze(-1),
                                             key_states.transpose(2, 3).unsqueeze(-1).transpose(-2, -1))
            attn_weights_temp = attn_weights.unsqueeze(2).expand(-1, -1, query_states.size()[-1], -1,
                                                                 -1) - attn_weights_temp
            attn_weights_temp = nn.functional.softmax(attn_weights_temp, dim=-1, dtype=torch.float32).to(
                query_states.dtype)
        ...
        # !
        if early_layers:
            attn_weights_temp = attn_weights_temp - attn_weights.unsqueeze(2).expand(-1, -1, query_states.size()[-1],
                                                                                     -1, -1)
            ...
            query_score, key_score, attn_output_temp, o_score = attn_weights_temp.squeeze().tolist(), attn_weights_temp.squeeze().tolist(), attn_output_temp, torch.sum(
                torch.abs(attn_output_temp), dim=1).squeeze().tolist()  # !
        ...
        if early_layers:
            return attn_output, attn_weights, past_key_value, query_score, key_score, torch.sum(
                torch.abs(attn_output_temp), dim=1).squeeze().tolist(), o_score
        else:
...
class Qwen2_5_VLDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        hidden_states, up_score, down_score = self.mlp(hidden_states, early_layers=early_exit,
                                                       deactivate_down=deactivate_keys_fwd_down,
                                                       deactivate_up=deactivate_keys_fwd_up)
        hidden_states = residual + hidden_states
...
class Qwen2_5_VLTextModel(Qwen2_5_VLPreTrainedModel):
    ...
    def forward(
        ...
        if not under_layer and not gen_layer:
            ...
            index_keys_reason = [item for item in index_keys if
                                 item not in index_keys_under and item not in index_keys_gen]
        # !
        ...
        for idx, decoder_layer in enumerate(self.layers[: self.config.num_hidden_layers]):
            ...
            if idx in index_keys_under:
                activate_keys_fwd_up = set(
                    itertools.islice(activate_keys_fwd_up_set[idx], ffn_number * int(whether_under_fwd)))
                activate_keys_fwd_down = set(
                    itertools.islice(activate_keys_fwd_down_set[idx], ffn_number * int(whether_under_fwd)))
                ...
                activate_keys_fwd_up = set(
                    itertools.islice(activate_keys_fwd_up_set[idx], ffn_number * int(whether_reason_fwd)))
                activate_keys_fwd_down = set(
                    itertools.islice(activate_keys_fwd_down_set[idx], ffn_number * int(whether_reason_fwd)))
                ...
                activate_keys_fwd_up = set(
                    itertools.islice(activate_keys_fwd_up_set[idx], ffn_number * int(whether_gen_fwd)))
                activate_keys_fwd_down = set(
                    itertools.islice(activate_keys_fwd_down_set[idx], ffn_number * int(whether_gen_fwd)))
...
class Qwen2_5_VLForConditionalGeneration(GenerationMixinCustom, Qwen2_5_VLPreTrainedModel):
    ...
    def forward(
        ...
        if early_exit_layers is not None:
            summed_data_fwd = {key: sum(value) for key, value in
                               hidden_scores_fwd_up.items()}  # key - layer num, value - list of scores over all imputs
            ...
            combined_data = {key: summed_data_fwd[key] * 3 + summed_data_q[key] * 2 + summed_data_v[key] * 2 for key in
                             summed_data_fwd}
