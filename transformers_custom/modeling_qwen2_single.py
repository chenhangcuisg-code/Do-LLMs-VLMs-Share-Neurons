...
def eager_attention_forward(
    ...
    if early_layers:
        attn_weights_temp = torch.matmul(query.transpose(2, 3).unsqueeze(-1),
                                         key_states.transpose(2, 3).unsqueeze(-1).transpose(-2, -1)) * scaling
    ...
    if early_layers:
        ...
        return attn_output, attn_weights, attn_weights_temp.squeeze().tolist(), attn_weights_temp.squeeze().tolist(), attn_output_temp, torch.sum(
...
class Qwen2Attention(nn.Module):
    ...
    def forward(
        ...
        if early_layers:
            ...
            # !
            return attn_output, attn_weights, query_score, key_score, torch.sum(torch.abs(attn_output_temp),
                                                                                dim=1).squeeze().tolist(), o_score
        else:
...
class Qwen2ForCausalLM(GenerationMixinCustom, Qwen2PreTrainedModel):
    ...
    #             return arr  # (B, N)
    #
    #         vec_fwd_up_bn = {k: _to_numpy_bn(v) for k, v in hidden_scores_fwd_up.items()}
    #         vec_fwd_down_bn = {k: _to_numpy_bn(v) for k, v in hidden_scores_fwd_down.items()}
    #         vec_q_bn = {k: _to_numpy_bn(v) for k, v in hidden_scores_q.items()}
    #         vec_k_bn = {k: _to_numpy_bn(v) for k, v in hidden_scores_k.items()}
    ...
    #
    #         combined_data = {
    #             key: (summed_data_fwd.get(key, 0.0) * 3.0
    #                   + summed_data_q.get(key, 0.0) * 2.0
    #                   + summed_data_v.get(key, 0.0) * 2.0)
    ...
    #             if k is None or k <= 0 or N == 0:
    #                 return _np.zeros((B, 0), dtype=_np.int64)
    #             k = min(k, N)
    #             out = []
    #             for b in range(B):
    #                 idx = _np.argsort(scores_bn[b])[-k:][::-1]
